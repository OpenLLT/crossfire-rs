...
impl<T> Rx<T> {
    ...
    pub(crate) fn _recv_blocking(
        shared: &ChannelShared<T>, deadline: Option<Instant>, waker_cache: Option<&WakerCache>,
    ...
    pub fn recv<'a>(&'a self) -> Result<T, RecvError> {
        Self::_recv_blocking(&self.shared, None, Some(&self.waker_cache)).map_err(|err| match err {
    ...
    pub fn recv_timeout(&self, timeout: Duration) -> Result<T, RecvTimeoutError> {
        match Instant::now().checked_add(timeout) {
            Some(deadline) => {
                Self::_recv_blocking(&self.shared, Some(deadline), Some(&self.waker_cache))
            }
