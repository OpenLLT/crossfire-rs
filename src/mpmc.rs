...
pub fn unbounded_blocking<T: Unpin>() -> (MTx<T>, MRx<T>) {
    let send_wakers = RegistrySender::Dummy(RegistryDummy::new());
    let recv_wakers = RegistryRecv::Multi(RegistryMultiRecv::new());
...
pub fn bounded_blocking<T: Unpin>(mut size: usize) -> (MTx<T>, MRx<T>) {
    ...
    let send_wakers = RegistrySender::Multi(RegistryMultiSender::new());
    let recv_wakers = RegistryRecv::Multi(RegistryMultiRecv::new());
